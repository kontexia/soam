...
class DistributedAMFabric:
    ...
    def search_for_bmu(self, fabric_uid: str, sdr: SDR, ref_id: Union[str, int], non_hebbian_edges=('generalise',), lock_fabric: bool = True) -> Optional[dict]:
        """
    ...
    def learn(self, fabric_uid: str, search_por: dict, persist: bool = True, lock_fabric: bool = True) -> Optional[dict]:
        """
        ...
        try:
            ...
            if persist:
                ...
                # first get current persist graph from cache
                # then get fabric to update it
                # then persist back into cache
                #
                fabric_key = 'AMFabric_{}'.format(fabric_uid)
                pg_future = self.cache.get_kv(store_name=self.config['fabric_graph'], key=fabric_key)
                pg = pg_future.result()
                pg_future = fabric.get_persist_graph(pg_to_update=pg, only_updated=True, ref_id=search_por['ref_id'])
                pg = pg_future.result()
                self.cache.set_kv(store_name=self.config['fabric_graph'], key=fabric_key, value=pg, persist=True)
    ...
    def train(self, fabric_uid: str, sdr: SDR, ref_id: Union[str, int], non_hebbian_edges=('generalise',), persist: bool = True, lock_fabric: bool = True) -> Optional[dict]:
        """
        ...
        try:
            ...
            if persist:
                ...
                pg_future = self.cache.get_kv(store_name=self.config['fabric_graph'], key=fabric_key)
                pg = pg_future.result()
                pg_future = fabric.get_persist_graph(pg_to_update=pg, only_updated=True, ref_id=por['ref_id'])
                pg = pg_future.result()
                self.cache.set_kv(store_name=self.config['fabric_graph'], key=fabric_key, value=pg, persist=True)
    ...
    def query(self, fabric_uid: str, sdr, top_n: int = 1, lock_fabric: bool = True) -> Optional[Dict[str, Union[List[str], NeuroColumn]]]:
        """
    ...
    def decode_fabric(self, fabric_uid: str, all_details: bool = False, community_sdr: bool = False, lock_fabric: bool = True) -> Optional[dict]:
        """
        ...
        :param community_sdr: If True the community sdr will be included
        :param lock_fabric: If True the fabric will be locked else if false it is assumed a fabric lock has already been acquired
        ...
        try:
            ...
            result = fabric.decode_fabric(all_details=all_details, community_sdr=community_sdr)
